#!/usr/bin/env bash
#
# PHOSPHENE <product-management> — emit DONE receipt (JSONL bus)
#
# Purpose:
...
usage() {
  cat <<'EOF'
Usage:
  ./.codex/skills/phosphene/cerulean/product-management/modulator/scripts/product-management_emit_done_receipt.sh --issue-number <N> --work-id <PRD-001>

Notes:
  - This writes to: phosphene/signals/bus.jsonl (append-only)
  - The emitted line will have:
      signal_type = phosphene.done.product-management.receipt.v1
...
parent_line="$(
  grep -F "\"signal_type\":\"phosphene.prism.product-management.branch_invoked.v1\"" "$BUS" \
...
run_marker="$WORK_ID"
output_key="done:receipt:product-management:issue:${ISSUE_NUMBER}"
...
line="{\"signal_version\":1,\"signal_id\":\"${signal_id}\",\"signal_type\":\"phosphene.done.product-management.receipt.v1\",\"work_id\":\"${WORK_ID}\",\"domain\":\"product-management\",\"issue_number\":${ISSUE_NUMBER},\"lane\":\"${lane}\",\"phos_id\":\"${phos_id}\",\"parents\":[\"${parent_signal_id}\"],\"run_marker\":\"${run_marker}\",\"output_key\":\"${output_key}\",\"ok\":true,\"created_utc\":\"${created_utc}\"}"

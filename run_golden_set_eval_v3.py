"""
Run Golden Set Evaluation with V3 Engine - Compare against V2 baseline

This script:
1. Loads 50 questions from GOLDEN_SET_QUESTIONS
2. Runs evaluation with V3 clause-aware chunking + query expansion
3. Writes all results to Snowflake EVAL_RESULTS and EVAL_RUNS tables
4. Compares against V2 baseline (70% pass rate)
"""
...
class GoldenSetEvaluatorV3:
    """Run golden set evaluation with V3 engine"""
    ...
    def run_evaluation(self, questions, run_name):
        ...
        """
        # Initialize V3 engine and evaluator
        self.engine = SparkieEngineV3(use_v3=True)
        ...
        print(f"\n{'='*80}")
        print(f"STARTING V3 EVALUATION: {run_name}")
        print(f"Engine: SparkieEngineV3 (Clause-aware + Query Expansion)")
        ...
        for i, q in enumerate(questions, 1):
            ...
            try:
                # Generate response using V3 engine
        ...
        print(f"\n{'='*80}")
        print(f"V3 EVALUATION COMPLETE: {run_name}")
        print(f"{'='*80}")
        ...
        print(f"{'='*80}")
        print(f"V2 Baseline Pass Rate: 70.0%")
        print(f"V3 Pass Rate:          {summary.get('pass_rate_pct', 0):.1f}%")
        improvement = (summary.get('pass_rate_pct', 0) or 0) - 70.0
        if improvement > 0:
    ...
    def _create_eval_run_record(self, eval_run_id, run_name, sample_size):
        """Create EVAL_RUNS record in Snowflake"""
        if not self.connection:
            return
        ...
        try:
            ...
            eval_config = {
                ...
                'schema': 'SPARKIE_V3_DB.CLAUSE_STORE',
                'features': ['clause_aware_chunking', 'query_expansion'],
                'evaluator_version': '3.0_clause_aware',
                'judge_model': 'meta-llama/llama-4-maverick-17b-128e-instruct'
            }
            ...
            """, (eval_run_id, run_name, 'golden_set_v3', sample_size, eval_config_json, 'running'))
    ...
    def _save_eval_result(self, result):
        """Save evaluation result to EVAL_RESULTS table"""
        if not self.connection:
            return
    ...
    def _complete_eval_run(self, eval_run_id, summary):
        """Update EVAL_RUNS with completion status"""
        if not self.connection:
            return
...
def main():
    """Main execution"""
    print("\n" + "="*80)
    print("GOLDEN SET EVALUATION - V3 CLAUSE-AWARE RAG")
    print("="*80 + "\n")

    evaluator = GoldenSetEvaluatorV3()
    ...
    # Run evaluation
    run_name = f"golden_set_v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
